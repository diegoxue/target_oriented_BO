setwd("C:\\Users\\12758\\Desktop\\target-result\\catylast\\alldata_fea\\1data\\repeat\\0.01\\size0.05")
...
size.i=c(round(0.05*nrow(data.training)))

iter_total=matrix(,1,8)
iter_mean=matrix(0,train.num,1)
iter_mean_w=matrix(0,train.num,1)
ave.OC=matrix(,nrow(iter)-1,4)
ave.dis=matrix(,nrow(iter)-1,4)
ave.OC_total=matrix(,nrow(iter)-1,8)
ave.dis_total=matrix(,nrow(iter)-1,8)
ave.OC_mean=matrix(0,nrow(iter)-1,1)
ave.dis_mean=matrix(0,nrow(iter)-1,1)
set.seed(11*size.i)
sample.num0<-vector()
for(ii in 1:1000){
  #s <-  sample(c(1:111,113:(nrow(data.training))),80,replace=F,prob=NULL) 
...
repeat{
  ...
  for(size in size.i)
  {
    ...
    for(i in 1:train.num)
    {
      ...
      if(min(abs(training.data[,"es"]-data.training[tar_num,"es"]))<=0.01*(max(data.training[,"es"])-min(data.training[,"es"])))
      {
        ...
        repeat{
          ...
          if(sum(EI)==0||(nrow(unique(EI))==1&nrow(EI)>1))
          {
            ...
            if(min(abs(training.data0[,"es"]-data.training[tar_num,"es"]))<=0.01*(max(data.training[,"es"])-min(data.training[,"es"])))
            {
              if(u>=nrow(iter)-2){
                break
              }else{
  ...
  {
    ave.OC[j,2*(estimator-1)-1]=mean(data.OC.SD[j,])
    if (length(unique(data.OC.SD[j,]))==1)
      ave.OC[j,2*(estimator-1)]=0 else
        ave.OC[j,2*(estimator-1)]=t.test(data.OC.SD[j,])$conf.int[2]-t.test(data.OC.SD[j,])$estimate
  }

  
  for(j in 1:nrow(data.dis))
  {
    ave.dis[j,2*(estimator-1)-1]=mean(data.dis[j,])
    if (length(unique(data.dis[j,]))==1)
      ave.dis[j,2*(estimator-1)]=0 else
        ave.dis[j,2*(estimator-1)]=t.test(data.dis[j,])$conf.int[2]-t.test(data.dis[j,])$estimate
  }
...
colnames(ave.dis)=c("pre_value","pre_value.sd","ego","ego.sd")
write.csv(ave.dis,"ave.dis.csv")
colnames(ave.OC)=c("pre_value","pre_value.sd","ego","ego.sd")
write.csv(ave.OC,"ave.OC.csv")
colnames(iter_total)=c("ego_u","ego_u.sd","pre_value","pre_value.sd","ego","ego.sd","ego","ego.sd")
